var OgcoincrowdSale = artifacts.require("OgcoincrowdSale");
var OgcoinToken = artifacts.require("OgcoinToken");

contract('OgcoincrowdSale', function(accounts) {
    it('should deploy the token and store the address', function(done){
        OgcoincrowdSale.deployed().then(async function(instance) {
    ...
    it('one ETH should buy 1700 Ogcoin Tokens in PreICO', function(done){
        OgcoincrowdSale.deployed().then(async function(instance) {
            const data = await instance.sendTransaction({ from: accounts[7], value: web3.toWei(1, "ether")});
            const tokenAddress = await instance.token.call();
            const ogcoinToken = OgcoinToken.at(tokenAddress);
            const tokenAmount = await ogcoinToken.balanceOf(accounts[7]);
            assert.equal(tokenAmount.toNumber(), 1700000000000000000000, 'The sender didn\'t receive the tokens as per PreICO rate');
            done();
       });
    });
    ...
    it('one ETH should buy 1250 Ogcoin Tokens in ICO', function(done){
        OgcoincrowdSale.deployed().then(async function(instance) {
            const data = await instance.sendTransaction({ from: accounts[2], value: web3.toWei(1.5, "ether")});
            const tokenAddress = await instance.token.call();
            const ogcoinToken = OgcoinToken.at(tokenAddress);
            const tokenAmount = await ogcoinToken.balanceOf(accounts[2]);
            assert.equal(tokenAmount.toNumber(), 1875000000000000000000, 'The sender didn\'t receive the tokens as per ICO rate');
            done();
       });
    });
